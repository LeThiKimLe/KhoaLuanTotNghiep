...
const updateProfile = createAsyncThunk('profile/update', async ({ updatedInfor }, thunkAPI) => {
    try {
        ...
        const cur_user = JSON.parse(localStorage.getItem('admin_user'))
        cur_user.user = response
        localStorage.setItem('admin_user', JSON.stringify(cur_user))
        return cur_user
    } catch (error) {
        const message =
            (error.response && error.response.data && error.response.data.message) ||
    ...
    try {
        ...
        axiosRefreshClient
            ...
            .then((response) => {
                ...
                localStorage.setItem('validAdminSession', 'true')
                window.dispatchEvent(new Event('storage'))
                return response.data.accessToken
            })
            .catch((err) => {
                localStorage.setItem('validAdminSession', 'false')
            })
    } catch (error) {
        const message =
            (error.response && error.response.data && error.response.data.message) ||
