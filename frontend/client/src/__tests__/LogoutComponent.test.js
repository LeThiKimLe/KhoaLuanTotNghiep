...
test('renders the logout confirmation dialog', () => {
    ...
    const { getByText, getByTestId } = render(
        <LogoutConfirmation onConfirm={onConfirm} onCancel={onCancel} type={type} />
    );
...
test('renders the logout confirmation dialog with an inform message', () => {
    ...
    // Assert that the dialog title is rendered correctly
    expect(getByText('Bạn vừa đổi mật khẩu, hãy đăng nhập lại với mật khẩu mới')).toBeInTheDocument();
});
...
test('renders the logout confirmation dialog with an interupt message', () => {
    ...
    // Assert that the dialog title is rendered correctly
    expect(getByText('Phiên đăng nhập của bạn bị gián đoạn. Hãy đăng nhập lại')).toBeInTheDocument();
});