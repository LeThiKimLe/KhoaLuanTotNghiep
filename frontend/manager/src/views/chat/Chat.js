...
const ChatBox = ({ chatData, handleSendMessage, avt }) => {
    ...
    const getMessageAvt = (mess) => {
        ...
        if (messState === 'outgoing') return user?.user?.systemManager?.img
...
const Chat = () => {
    ...
    const handleConnectError = (error) => {
        ...
        const retry = localStorage.getItem('retry_socket_manager')
        if (retry === 'true') {
            return
        } else {
            ...
            dispatch(authThunk.getNewAccessToken())
                ...
                .then(() => {
                    ...
                    localStorage.setItem('retry_socket_manager', 'true')
    ...
    useEffect(() => {
        ...
        connection.current.addEventListener('open', (event) => {
            //remove manager_retry from local storage
            localStorage.removeItem('retry_socket_manager')
        })
            ...
            >
                <ConversationHeader>
                    <Avatar
                        ...
                        src={
                            user?.user?.systemManager?.img ? user?.user?.systemManager?.img : guest
                        }
                        status="available"
                    />
