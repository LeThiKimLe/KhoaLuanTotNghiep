...
public class CompanyChatService implements WebSocketHandler {
    ...
    private static final HashMap<Integer, List<WebSocketSession>> companySessions = new HashMap<>();
    @Autowired
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        Integer companyId = (Integer) attributes.get("id");
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
        ...
        if (role == 0) {
            ...
            // Gửi tin nhắn lại cho tất cả các máy nhân viên có cùng id kết nối
            for (WebSocketSession webSocketSession : companySessions.get(id)) {
                if (staffSessions.contains(webSocketSession))
            ...
            if (receivedMessage.startsWith("Close: ")) {
                ...
                // Gửi tin cho các máy nhân viên khác cùng id
                for (WebSocketSession webSocketSession : companySessions.get(id)) {
                    if (staffSessions.contains(webSocketSession) && webSocketSession != session)
                        webSocketSession.sendMessage(new TextMessage(sessionId + ": " + receivedMessage));
                }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception {
        ...
        companySessions.get(id).remove(session);
    }
